...
export default function App() {
  ...
  const goToPrevImage = () => {
    setSelectedImageIndex(prevIndex => (prevIndex === 0 ? response.length - 1 : prevIndex - 1));
  };

  const goToNextImage = () => {
    setSelectedImageIndex(prevIndex => (prevIndex === response.length - 1 ? 0 : prevIndex + 1));
  };
  ...
  return (
    <div>
      ...
      {response.length > 0 && <ImageGallery result={response} openModal={openModal} />}
      {loading && <Loader />}
